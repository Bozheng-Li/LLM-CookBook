...
# 1. Synaptic Intelligence (SI) & Online Path Integral of Parameter Gradients (Zenke et al.)
# 2. Dark Experience Replay (DER++) & Knowledge Distillation under Continual Streams
# 3. Model Merging & Task Arithmetic in LLM Continuous Lifelong Evolution (Ties-Merging)
...
class DarkExperienceReplayLoss(nn.Module):
    ...
    def forward(self, model_outputs_new: torch.Tensor, targets_new: torch.Tensor,
        ...
        \"\"\"
        计算 DER++ 联合损失: 新任务交叉熵 + 旧样本暗知识蒸馏 + 旧样本标签交叉熵
        \"\"\"
        # 1. 当前批次新任务损失
...
expansion_3 = """
    ...
    <div class="tbl-wrap">
      <table>
        ...
        <tbody>
          <tr><td><strong>线性平均加权 (Linear Merging)</strong></td><td>$\Theta = \Theta_{\text{Base}} + \sum_k w_k \tau_k$</td><td>简单暴力，若两任务在同一参数上更新方向相反，产生破坏性干涉（Destructive Interference）</td><td>仅适用于差异极小的同源微调模型</td></tr>
          ...
          <tr><td><strong>DARE (Yu et al., 2024)</strong></td><td>随机将 90%~99% 的微调参数丢弃（Drop），并通过 $1/(1-p)$ 动态缩放剩余参数</td><td>参数极度稀疏化，不同任务向量在几何空间中几乎天然正交，互不干扰</td><td>支持将上百个独立专业 LoRA 插件无损拼合进单一巨型主干</td></tr>
        </tbody>
      </table>
...
if insert_target in text:
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(new_text)
    print("Successfully expanded ch097.html with SI, DER++ and Task Arithmetic")
else:
