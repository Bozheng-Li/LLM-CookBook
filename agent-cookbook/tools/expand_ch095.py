...
path = r"D:/agent-cookbook/chapters/ch094.html" # Note: need to expand ch095.html!
path = r"D:/agent-cookbook/chapters/ch095.html"
with open(path, "r", encoding="utf-8") as f:
    text = f.read()

# Expand with comprehensive deep theoretical sections:
# 1. Symlog Predictions & Two-Hot Encoding (DreamerV3 Mathematics in Detail)
# 2. V-trace & Generalized Advantage Estimation (GAE) within Latent Imagined Trajectories
# 3. Complete Latent Imagination Policy Optimization Loop Implementation in Python
...
expansion_1 = """
    ...
    <p>传统的均方误差回归（MSE Loss, $\frac{1}{2}(y - \hat{y})^2$）在遇到极端离群点时，会产生高达误差平方倍数的巨额梯度，瞬间冲垮神经网络。DreamerV3 彻底抛弃了标量回归，将价值函数 $V$ 与奖励预测 $r$ 离散化为 $B = 255$ 个等间距的 Symlog 桶（Buckets）。对于任意连续目标值 $y$，其被编码为与其相邻的两个离散桶上的凸组合（Convex Combination）：</p>
...
class LatentDreamerTrainer:
    ...
    def imagine_ahead(self, initial_h: torch.Tensor, initial_z: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        for step in range(self.horizon):
            # 1. 策略网络根据当前潜状态决策动作 a ~ pi(a | h, z)
            state_feat = torch.cat([curr_h, curr_z], dim=-1)
            action = self.actor(state_feat) # 采用重参数化采样维持梯度可导
            action_seq.append(action)

            # 2. 闭环推进世界模型先验 (无物理观测介入)
        ...
        return {
            "h": torch.stack(h_seq, dim=0),      # shape: (H+1, B, deter_dim)
            "z": torch.stack(z_seq, dim=0),      # shape: (H+1, B, stoch_dim)
            "actions": torch.stack(action_seq, dim=0) # shape: (H, B, act_dim)
        }

    def compute_lambda_returns(self, rewards: torch.Tensor, values: torch.Tensor) -> torch.Tensor:
        \"\"\"逆向递归计算梦境轨迹的 GAE / lambda-return 目标\"\"\"
        # rewards: shape (H, B), values: shape (H+1, B)
        H = rewards.shape[0]
        ...
        return returns[:-1] # 返回前 H 步的优势目标</code></pre>
    </div>
"""
...
if insert_target in text:
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(new_text)
    print("Successfully expanded ch095.html with Symlog, Dreamer loop and Frontiers")
else:
